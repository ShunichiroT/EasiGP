...
specification:
    ...
    - keep only nodes whose "type" is exactly "GENE"
...
def extract_candidate_genes(network):
    ...
    only the nodes whose 'type' is exactly 'GENE'.

    Returns the raw node dicts (id, type, full_name, description, is_source,
    ...) unchanged, so no information from the source JSON is lost at this
    stage - filtering by type is the only operation performed here.
    """
    nodes = network['nodes']
    genes = [n for n in nodes if n.get('type') == 'GENE']

    if len(genes) == 0:
        warnings.warn("No nodes with type == 'GENE' were found in the network JSON.")

    non_gene_types = sorted({n.get('type') for n in nodes if n.get('type') != 'GENE'})
    if non_gene_types:
...
def build_gene_adjacency(network, gene_list, include_mediated_edges=False, max_hops=3):
    ...
    *name* level: row i of gene_list <-> index i of the adjacency matrix.
    ...
    node_type = {n['id']: n.get('type') for n in network['nodes']}

    direct_edges = []  # (src_name, tgt_name, sign, effect, mechanism)
    for e in network['edges']:
        direct_edges.append((e['source'], e['target'], e.get('sign'), e.get('effect'), e.get('mechanism')))
    ...
    for src_name, tgt_name, sign, effect, mechanism in direct_edges:
        if node_type.get(src_name) == 'GENE' and node_type.get(tgt_name) == 'GENE':
    ...
    if edge_list.shape[0] == 0:
        warnings.warn(
            ...
            "symbols in gene_list match the 'source'/'target' ids used in the network JSON."
        )
